							 ...
							 void *cap, vfs_context_t ctx)
{
	...
	if (ATTR_CMN_NAME & attr) {
		if (vap->va_name) {
			...
			if (!vnode_isvroot(vp)) {
				if ((zp->z_links > 1) && (IFTOVT((mode_t)zp->z_mode) == VREG) &&
					zp->z_finder_hardlink_name[0]) {
					copy_name_attr(vap, vp, zp->z_finder_hardlink_name,
								   strlen(zp->z_finder_hardlink_name));
				} else {
					uint64_t parent;
	...
	if (ATTR_CMN_CRTIME & attr) {
		uint64_t value;
		/* FIXME Bulk lookup is faster */
		error = sa_lookup(zp->z_sa_hdl, SA_ZPL_CRTIME(zfsvfs),
						  &value, sizeof (value));
	...
	if (ATTR_CMN_MODTIME & attr) {
		uint64_t value;
		/* FIXME Bulk lookup is faster */
		error = sa_lookup(zp->z_sa_hdl, SA_ZPL_MTIME(zfsvfs),
						  &value, sizeof (value));
	...
	if (ATTR_CMN_CHGTIME & attr) {
		uint64_t value;
		/* FIXME Bulk lookup is faster */
		error = sa_lookup(zp->z_sa_hdl, SA_ZPL_CTIME(zfsvfs),
						  &value, sizeof (value));
	...
	if (ATTR_CMN_ACCTIME & attr) {
		uint64_t value;
		/* FIXME Bulk lookup is faster */
		error = sa_lookup(zp->z_sa_hdl, SA_ZPL_ATIME(zfsvfs),
						  &value, sizeof (value));
